...
export function useInterview({ saveAttempt, practiceMode = false }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      axios.post('http://localhost:4000/evaluate-answer', {
        answer: textData,
        question: currentQ,
        questionIdx: qIdx,
      }).then(({ data }) => {
  ...
  const handleGenerateQuestions = async () => {
    ...
    try {
      const { data } = await axios.post('http://localhost:4000/generate-questions', { role, difficulty });
  ...
  const handleAudioUpload = async () => {
    ...
    try {
      const { data: uploadResponse } = await assemblyApi.post('/upload', audioDetails.blob);
